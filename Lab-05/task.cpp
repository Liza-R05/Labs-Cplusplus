...
public:
    ...
    // преобразование класса в одномерный массив
    operator int* () {
        return arr;
    }
...
int main() {
    ...
    cout << "\n7. Оператор ++ :\n";
    ++arr; // автоматически вызывает метод operator++() для объекта arr
    arr.output();
    ...
    arr.setMultiply(3); // на сколько умножить
    arr.output();

    cout << "\n10. Оператор * (умножение на 2):\n";
    IntArray arr2 = arr * 2; // на сколько умножить
    arr2.output();
    ...
    int source[] = { 10, 20, 30, 40, 50 }; // задается
